...
class KL(FDivergence):
  ...
  def _dual_estimate_with_logits(self, logits_p, logits_b):
    clip_logits_b = tf.tanh(logits_b)
    clip_logits_p = tf.tanh(logits_p)
    return clip_logits_b - tf.exp(clip_logits_p - 1)

  def _primal_estimate_with_densities(
      self, apn_logp, apn_logb, abn_logp, abn_logb):
    return tf.reduce_mean(apn_logp - apn_logb, axis=0)

# class rKL(FDivergence):
#   """Reverse KL divergence."""

#   def _dual_estimate_with_logits(self, logits_p, logits_b):
#     return -tf.exp(-logits_b) - (-1 + logits_p)
...
class Pearson(FDivergence):
  ...
  def _dual_estimate_with_logits(self, logits_p, logits_b):
    return logits_b - (1/4.0 * tf.square(logits_p) + logits_p)
  
# class Squared_Hellinger(FDivergence):
#   """Squared Hellinger divengence."""

#   def _dual_estimate_with_logits(self, logits_p, logits_b):
#     return 1 - tf.exp(-logits_b) - (1 - tf.exp(-logits_p)) / tf.exp(-logits_p)
  
# class Jensen_Shannon(FDivergence):
#   """Jensen-Shannon divengence."""

#   def _dual_estimate_with_logits(self, logits_p, logits_b):
#     return tf.log(2) - tf.log(1 + tf.exp(-logits_b)) - (- tf.log(2 - 2 / (1 + tf.exp(-logits_p))))

# class gan(FDivergence):
#   """GAN divengence."""

#   def _dual_estimate_with_logits(self, logits_p, logits_b):
#     return -tf.log(1 + tf.exp(-logits_b)) - (- tf.log(1 - 1 / (1 + tf.exp(-logits_p)))) 
